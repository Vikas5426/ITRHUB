...
export function DeadlineGrid({ deadlines, onMarkDone, doneDeadlines }: { deadlines: Deadline[], onMarkDone: (id: string) => void, doneDeadlines: Set<string> }) {
  ...
  const getStatusColor = (status?: string) => {
    switch (status) {
      ...
      case "Due Soon": return "bg-amber-100 text-amber-600";
  ...
  return (
    <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-gray-200 h-full">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <h3 className="text-xl font-bold">All Deadlines</h3>
        ...
        {filteredDeadlines.map((deadline) => {
          ...
          return (
            <motion.div 
              layout
              key={deadline.id}
              className={`border rounded-2xl p-5 transition-all cursor-pointer ${isDone ? 'bg-gray-50 border-gray-100' : 'bg-white border-gray-200 hover:border-gray-300 hover:shadow-sm'}`}
              onClick={() => setExpanded(isExpanded ? null : deadline.id)}
            >
              <div className="flex justify-between items-start mb-3">
                <div>
                  <div className="text-xl font-black text-black mb-1">{deadline.displayDate}</div>
                  <div className={`text-sm font-bold ${isDone ? 'text-gray-400 line-through' : 'text-gray-700'}`}>{deadline.name}</div>
                </div>
              ...
              <div className="flex flex-wrap gap-1.5 mb-4">
                {deadline.applicableProfiles.map(p => (
                  <span key={p} className="text-[10px] font-bold bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full">
                    {p}
                  </span>
                ))}
              </div>
              ...
              <AnimatePresence>
                {isExpanded && (
                  ...
                  >
                    <div className="pt-3 border-t border-gray-100 mt-2 mb-4">
              ...
              <div className="flex items-center gap-2 mt-auto pt-2" onClick={e => e.stopPropagation()}>
                ...
                <button className="flex-1 flex justify-center items-center gap-1.5 py-2 rounded-xl text-xs font-bold bg-gray-100 text-gray-700 hover:bg-gray-200 transition-colors">
                  <Bell size={14} />
                  Set Reminder
                </button>
              </div>
